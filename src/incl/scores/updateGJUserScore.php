...
if (!$legacyID) exit($gs->getUserID($id));
$userID = $gs->getUserID($legacyID);
$uploadDate = time();
$hostname = $gs->getIP();

if(!isset($_POST["secret"]) OR !isset($_POST["stars"]) OR !isset($_POST["demons"]) 
	OR !isset($_POST["icon"]) OR !isset($_POST["color1"]) OR !isset($_POST["color2"]))
{
	exit("-1");
}

$userName = $gs->getAccountName($legacyID);
...
if(!empty($dinfo)) {
	...
	$demonsCount = $demonsCount->fetchAll()[0];
...
// recording new progression for weekly leaderboards
$query2 = $db->prepare("INSERT INTO actions (type, value, timestamp, account, value2, value3, value4, value5, value6) VALUES ('9', :stars, :timestamp, :account, :coinsd, :demon, :usrco, :diamond, :moons)");
